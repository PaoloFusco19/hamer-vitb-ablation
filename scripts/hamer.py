...
class HAMER(pl.LightningModule):
    ...
    def __init__(self, cfg: CfgNode, init_renderer: bool = True):
        ...
        if init_renderer:
            self.renderer = SkeletonRenderer(self.cfg) if SkeletonRenderer is not None else None
            self.mesh_renderer = MeshRenderer(self.cfg, faces=self.mano.faces) if MeshRenderer is not None else None
        else:
            self.renderer = None
